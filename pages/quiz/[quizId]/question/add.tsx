...
export default function AddQuestion({ quizId }: { quizId: string }) {
    ...
    const [hasJokerValue, setHasJokerValue] = useState(false);
    const [questionValue, setQuestionValue] = useState(2);
    const [joker, setJoker] = useState<Jokers>(Jokers.Wikipedia);
    const [mediaContent, setMediaContent] = useState("");
    const [imageFileList, setImageFileList] = useState<UploadFile[]>([]);
    const [solutionType, setSolutionType] = useState<SolutionTypes>(SolutionTypes.Text);
    ...
    const [sortItems, setSortItems] = useState<{ name: string, value: number}[]>([{ name: "", value: 0 }]);
        ...
        <div style={{ background: "#222", width: "100vw", height: "100vh", color: "white", display: "flex", alignItems: "center", gridTemplateColumns: "60% 40%", overflow: "hidden" }}>
            <Form name="Add Question" style={{ padding: 10, color: "white", fontSize: "2rem", width: "60%" }} {...formItemLayout}>
                ...
                <Form.Item label="Question Type" className={styles.form}>
                    <Select defaultValue={questionType} onChange={(value) => {
                ...
                <Form.Item name="caption" label="Caption" className={styles.form}>
                    <Input.TextArea onChange={(event) => setCaption(event.target.value)} value={caption} placeholder={"Welche der folgendenen..."} maxLength={100} />
                </Form.Item>
